...
### コンセプト・About 本文/見出しでの改行テクニック

**見出し・本文どちらの textarea でも Enter で改行できます** (PC・モバイル両方で改行)。HTML タグは特殊ケース用。

| やりたいこと | 書き方 |
|---|---|
| 通常の改行 (PC もモバイルも改行) | textarea で **Enter 1 回** |
| 段落区切り (28px 余白) | textarea で **Enter 2 回** (空行を 1 つ作る) |
| **モバイル (≤768px) でだけ改行** | 該当箇所に `<br class="br-mobile" />` |
| **デスクトップ (>768px) でだけ改行** | 該当箇所に `<br class="br-desktop" />` |

例:
```
ある瞬間の「記録」である。<br class="br-mobile" />私たちはそう考えます。
```

→ PC: 「ある瞬間の「記録」である。私たちはそう考えます。」(1 行)
→ モバイル: 改行されて 2 行表示
...
Stripe ダッシュボード → 商品 → **その他のオプション → メタデータ** で設定。サイト側で表示・挙動が変わります。

| キー | 用途 | 値の例 |
|---|---|---|
| `creator` | 作家名。商品カードのサブ表示・フィルター・検索 | `Tsutomu Yoshizaki` |
| `tag` | カードと詳細ページに付くオレンジ色のラベル | `NEW`, `限定`, `再入荷` |
| `series` | シリーズ名。Archive にシリーズ別フィルターが追加される (2 種類以上で表示) | `Spring 2026` |
| `paperType` | 詳細ページのメタ情報に「用紙: ◯◯」を追加 | `マット` `光沢` |
| `size` | 詳細ページのサイズ表記を上書き (空ならグローバル設定) | `100 × 148 mm` |
| `description2` | Stripe の description の下に薄い色で追加表示 | `※ 写真は実物と多少異なる場合があります` |
| `releaseDate` | 詳細ページの価格下に表示 | `2026.05 リリース` |
| `featured` | `true` にすると Picks (トップ) で先頭に並ぶ | `true` |
| `soldOut` | `true` にするとカードと詳細で「Sold Out」表示・購入不可 (Stripe 側 active のままで OK) | `true` |
| `image2` 〜 `image8` | 商品詳細ギャラリーの 2 枚目以降 | URL |

**真偽値は文字列で**: `featured = true` (✅ Stripe 上は文字列でも動く)。`True` `1` `yes` などはダメ — 小文字の `true` のみ。

---

## 🥬 野菜直売所形式 (Payment Link + QR コード POP)

エリアに POP を設置して QR コードでセルフ決済してもらう運用。**性善説**ベースなので在庫は物理的に補充された分だけ。

### 1. Payment Link の作成 (エリアごと)

1. Stripe ダッシュボード → **Payment Links** → **+ New Link**
2. 商品を選択 (or その場で作成)、価格を確認
3. **「その他のオプション」を展開**:
   - **Quantity adjustable**: OFF (固定数量)
   - **Collect customer info**: 名前 + メール (任意でアンケート)
4. **メタデータ** (重要!):

| キー | 値 (エリア識別子) | 例 |
|---|---|---|
| `location` | エリアの英数字 ID | `kanazawa-station` |
| `locationName` | 表示用の日本語名 (任意) | `金沢駅前カフェ` |
| `productName` | 商品名 (集計表示用) | `ポストカード A セット` |

5. **Save** → 発行された URL (`https://buy.stripe.com/...`) をコピー

### 2. QR コードの生成 + POP 印刷

1. [qrcode-monkey.com](https://www.qrcode-monkey.com/) など QR 生成サイトに Payment Link URL を貼る
2. ロゴ入り・色付き QR を作成 → ダウンロード (PNG / SVG)
3. POP デザイン (Canva 等) で:
   - 商品写真
   - 価格
   - QR コード (3〜5cm 推奨)
   - 「スマホのカメラで読み取って決済 → 商品お持ち帰り」のガイド
   - 設置場所名 (お礼として)
4. ラミネート加工して設置
...
Payments → フィルター → **メタデータ** → `location` で絞り込み。期間別売上が見える。

#### 自前ダッシュボード (この後 Webhook + KV を導入したら使える)

`https://log-letter.com/admin/stands` で:
- エリア別売上合計
- エリア別販売数
- 最新の販売時刻
- 期間集計
...
**初期 (手動)**: 物理的に補充した数だけ在庫がある。Stripe 側の在庫管理はせず、無くなったら補充するだけ。

**自動化したい場合**: 各エリアの初期在庫を decide → Webhook で減算 → 残量が閾値を切ったら通知メール。これは `/api/stripe-webhook` を拡張する必要あり (相談ください)。

### 6. 自前ダッシュボード `/admin/stands` の有効化 (初回のみ)

**Cloudflare ダッシュボードでの設定**:

1. https://dash.cloudflare.com → Workers & Pages → **KV** → **Create namespace**:
   - Name: `STANDS_KV` (好きな名前で OK)
   - 作成後、表示される **Namespace ID** をコピー
2. **Workers & Pages → log-letter プロジェクト** → **Settings → Bindings** (or Variables and Secrets):
   - **+ Add binding** → **KV Namespace**
   - Variable name: `STANDS_KV`
   - KV namespace: 先ほど作ったものを選択 → Save
3. **Settings → Environment variables** → 以下を追加:
   - `STANDS_ADMIN_PASSWORD` = `任意のパスワード` (例: `loglet-stands-2026`) — `/admin/stands` の Basic 認証用
4. **Deploy** ボタンで再デプロイ
...
5. Cloudflare → log-letter → Environment variables → `STRIPE_WEBHOOK_SECRET` に貼り付け → 再デプロイ

**動作確認**:

1. テスト用 Payment Link を 1 つ作成 (`location: test-stand` メタデータ付き)
2. テスト決済 (Stripe テスト環境のカード `4242 4242 4242 4242`)
3. `https://log-letter.com/admin/stands` にアクセス → Basic 認証でユーザー名空・パスワード入力
4. **エリア別** に「test-stand」が出ていれば OK

### 7. 集計画面の使い方

`/admin/stands`:
- **合計売上 / 販売数 / エリア数** が最上部に
- **エリア別テーブル**: ロケーション・販売数・売上合計・最新販売時刻
- **直近 50 件**: 個別の販売記録
- **期間フィルター**: From/To で絞り込み (例: 月次集計)

### 5. トラブル対応

- **QR が破れた**: 同じ Payment Link URL を再生成で OK (URL 変わらない)
- **金額を変えたい**: Stripe ダッシュボードで Payment Link を編集 (新しい価格を作って差し替え)
- **設置場所を増やす**: 新しい Payment Link を作って `location: 〇〇` メタデータで識別。POP 印刷
- **設置場所を移転**: 同じ Payment Link でも OK (URL は変わらない)。集計上の location は固定なので、新エリアなら新 Link 推奨
